...
import numpy as np
from . import scoring
from .dataloader.goboard_9x9 import GoBoard
from .processor9x9 import ThreePlaneProcessor
...
class HTTPFrontend(object):
    ...
    def run(self):
        ...
        def home():
            ...
            return open("ui/demoBot_9x9.html").read().replace('"__i__"',
                                                          'var boardInit = ' + board_init)  # output the modified HTML file
...
class KerasBot(GoModel):
    ...
    def _model_moves(self, bot_color):
        ...
        for idx in top_n_pred_idx:
            prediction = int(idx)
            pred_row = prediction // self.board_size
            pred_col = prediction % self.board_size
...
class RandomizedKerasBot(GoModel):
    ...
    def _model_moves(self, bot_color):
        ...
        pred = pred * pred * pred
        pred /= pred.sum()
        moves = np.random.choice(self.board_size * self.board_size, size=n_samples, replace=False, p=pred)
