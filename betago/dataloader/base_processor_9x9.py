...
from .. import gosgf
from .goboard_9x9 import GoBoard
from .index_processor import KGSIndex
from .sampling_9x9 import Sampler
...
class DataGenerator(object):
    ...
    def get_num_samples(self, batch_size=128, nb_classes=board_size * board_size):
    ...
    def generate(self, batch_size=128, nb_classes=board_size * board_size):
        while True:
...
class GoBaseProcessor(object):
    ...
    def load_go_data_9x9(self, types=['train'], data_dir='data_9x9', num_samples=1000):
        '''
        Main method to load go data.
    ...
    def draw_ogs_samples(self):


        sampler = Sampler(data_dir=self.data_dir)
        samples = sampler.draw_training_samples(5)
...
class GoDataProcessor(GoBaseProcessor):
    ...
    def process_zip(self, dir_name, zip_file_name, data_file_name, game_list):
        ...
        features = np.zeros((total_examples, self.num_planes, board_size, board_size))
...
class GoFileProcessor(GoBaseProcessor):
    ...
    def process_zip(self, dir_name, zip_file_name, data_file_name, game_list):
        ...
        self.write_file_header(data_file=data_file, n=total_examples, num_planes=7, board_size=board_size, bits_per_pixel=1)
    ...
    def consolidate_games(self, name, samples):
        ...
        self.write_file_header(consolidated_file, num_records, self.num_planes, board_size, 1)
